function empty_table = create_empty_table( datasets_number, data_info, save_path) 
    ...
    %
    % Description: Creates an empty table with specified format for storing dataset
    %              information.
    %
    % Syntax:
    ...
    %                         It is expected to have 11 columns with data of the
    %                         following types, used to build the following table
    %                         columns (note that double vars must be positive).
    %
    ...
    %
    % Output: 
    %   - empty_table (table): Table with the specified format for storing dataset
    %                          information.
    %
    ...
    if isempty(data_info)
        ...
        [N, M] = size(data_info);
        if M ~=11 || N > datasets_number
            error([ ...
                "incompatible size of given data info." ...
                " Data_info must have size(1) < datasets_number and size(2) == 11. " ...
        ...
        for i = 1:11
            if i == 1 || i==10 || i==11
                if ~isnumeric(data_info{1,i}) || ...
                        ...
                        isinf(data_info{1,i})
                    error( ...
                        "wrong data type used in the given_data info. " + ...
                        " See help for more info" ...
                    )
                end
            else
                if ~ischar(data_info{1,i})
                    error( ...
                        "wrong data type used in the given data_info." + ...
                        " See help for more info" ...
                    )
                end
            end
        ...
        if N == datasets_number
            ...
            for i = 1:N
                for k = 1:M
                    if k==1 || k==10 || k==11
                        if data_info{i,k} < 0
                            error(['got a negative value for ' variable_names_types])
                        end
