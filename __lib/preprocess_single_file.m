...
%
% Usage example:
%   [EEG, L] = preprocess_single_file('/path/to/raw/', 'raw_data.eeg', 'channel_info.tsv', 'preprocessed_data.set', ...
%                                     channel_systems_struct, data_info_struct, [], params_info_struct, template_info_struct, L_struct, save_info_struct);
%
% Notes:
    ...
    if ~isempty(EEG)
        ...
        if params_info.prep_steps.ASR
            ...
            if max(max(abs(EEG.data)))>params_info.th_reject 
                fprintf([' \t\t\t\t\t\t\t\t --- ' num2str(c+1) '° ASR APPLIED ---\n']);
