...
% Check number of files requested if we are preprocessing an entire dataset
    if ischar(numbers_files.N_subj)
        if ~isequal(numbers_files.N_subj,'all')
            error('ERROR: INVALID STRING, ONLY "all" CAN BE INSERTED');
        end
    elseif numbers_files.N_subj<=0
        error('ERROR: INSERTED NEGATIVE NUMBER IN numbers_files.N_subj');
    end

    if ischar(numbers_files.N_sess)
        if ~isequal(numbers_files.N_sess,'all')
            error('ERROR: INVALID STRING, ONLY "all" CAN BE INSERTED');
        end
    elseif numbers_files.N_sess<=0
        error('ERROR: INSERTED NEGATIVE NUMBER IN numbers_files.N_sess');
    end

    if ischar(numbers_files.N_obj)
        if ~isequal(numbers_files.N_obj,'all')
            error('ERROR: INVALID STRING, ONLY "all" CAN BE INSERTED');
        end
    elseif numbers_files.N_obj<=0
