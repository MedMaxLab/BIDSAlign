...
function [C] = create_chan_loc(EEG, data_info, list_ref, template_info)
    ...
    %   - list_ref (list): List of channel names or indices for creating channel locations.
    %   - template_info (struct): Structure containing template information.
    %
    % Output:
    %   - C (struct): Structure containing channel locations.
    %
    % Notes:
    ...
    if isempty(list_ref) 
        ...
        else %standard 10-20, 10-10, 10-5 
            listZ  = list_ref;
            NchanZ = length(listZ);
        end
    end

    %% At the end both listZ and listS, have channels named in 10-10 standard
    C = S([]);  
    for i=1:NchanZ
