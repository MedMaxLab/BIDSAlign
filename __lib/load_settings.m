function varargout = load_settings( setting_name, info_type )
    ...
    %
    % Description: Loads settings information based on the
    %              specified setting name and info type.
    %
    % Syntax:
    ...
    if ~ischar(info_type)
        error(['info_type must be a char array in ' ...
            ' [path | save | preprocess | selection | all ]'])
    else
        if ~any(strcmpi(info_type, {'path' 'preprocess' 'selection' 'all' 'save'}))
            error(['info_type must be a char array ' ...
                'in [path | save | preprocess | selection | all ]'])
        end
    end
    
    struct_name = {'path_info.mat', 'preprocessing_info.mat', ...
        'selection_info.mat', 'save_info.mat'};
    if  strcmpi(info_type,'all')
        field_name = {'path_info', 'params_info', 'selection_info', 'save_info'};
        varargout = cell(1,4);
        for i = 1:4
            try
                struct_info = load([ filePath 'default_settings' filesep ...
                    setting_name filesep struct_name{i} ]).(field_name{i});
        ...
        if iscellstr(varargout)
            d = dir( [ filePath 'default_settings' filesep setting_name ]);
            d = d(~ismember({d(:).name},{'.','..'}));
            if isempty(d)
                warning(" setting name exist but doesn't have " + ...
                    "any valid file and is empty. Removing it to avoid bugs.")
                remove_settings(setting_name);            
            else
                warning([' setting name exist but does not have any valid file. '...
                    'It is suggested to copy all its content elsewhere ' ...
                    ' and remove the setting name.'])
            end                
        end
    else
        %nargout=1;
